    ...
    def generate_work_order_number(self):
        ...
        if not self.work_order_number:
            ...
            count = WorkOrder.query.filter(
                db.func.date(WorkOrder.created_at) == datetime.now().date()
            ).count() + 1
            self.work_order_number = f'WO{date_str}{count:04d}'
...
class InspectionTask(db.Model):
    ...
    def generate_task_number(self):
        ...
        if not self.task_number:
            date_str = datetime.now().strftime('%Y%m%d')
            count = InspectionTask.query.filter(
                db.func.date(InspectionTask.created_at) == datetime.now().date()
            ).count() + 1
            self.task_number = f'IT{date_str}{count:04d}'
...
class InspectionResult(db.Model):
    ...
    def generate_result_number(self):
        ...
        if not self.result_number:
            date_str = datetime.now().strftime('%Y%m%d')
            count = InspectionResult.query.filter(
                db.func.date(InspectionResult.created_at) == datetime.now().date()
            ).count() + 1
            self.result_number = f'IR{date_str}{count:04d}'
    ...
    def generate_change_number(self):
        ...
        if not self.change_number:
            ...
            count = ChangeRequest.query.filter(
                db.func.date(ChangeRequest.created_at) == datetime.now().date()
            ).count() + 1
            self.change_number = f'CR{date_str}{count:04d}'
...
class ChangeApproval(db.Model):
    ...
    change = db.relationship('ChangeRequest', backref=db.backref('approval_chain', cascade='all, delete-orphan'))

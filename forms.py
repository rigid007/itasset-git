...
class VlanForm(FlaskForm):
    vlan_id = IntegerField('VLAN ID', validators=[
        DataRequired(message='请输入 VLAN ID'),
        NumberRange(min=1, max=4094, message='VLAN ID 必须在 1-4094 之间')
    ])
    name = StringField('VLAN 名称', validators=[DataRequired(message='请输入 VLAN 名称'), Length(max=128)])
    vlan_type = SelectField('VLAN 类型', choices=[
        ('业务', '业务'), ('管理', '管理'), ('互联', '互联'),
        ('无线', '无线'), ('存储', '存储'), ('其他', '其他')
    ], default='业务')
    gateway = StringField('网关', validators=[Optional(), Length(max=45)])
    description = TextAreaField('描述', validators=[Optional()])
    status = SelectField('状态', choices=[
...
class SubnetForm(FlaskForm):
    name = StringField('子网名称', validators=[DataRequired(message='请输入子网名称'), Length(max=128)])
    network = StringField('网络地址', validators=[DataRequired(message='请输入网络地址，如 192.168.1.0')],
                          render_kw={'placeholder': '192.168.1.0'})
    prefix_length = IntegerField('前缀长度', validators=[
        DataRequired(message='请输入前缀长度'),
        NumberRange(min=1, max=30, message='前缀长度必须在 1-30 之间')
    ], default=24)
    gateway = StringField('网关', validators=[Optional(), Length(max=45)],
                          render_kw={'placeholder': '如 192.168.1.1'})
    vlan_id = SelectField('所属 VLAN', coerce=int, validators=[Optional()])
    location_id = SelectField('所在位置', coerce=int, validators=[Optional()])
    department = StringField('所属部门', validators=[Optional(), Length(max=100)])
    tags = StringField('标签', validators=[Optional(), Length(max=255)],
    ...
    ipam_enabled = BooleanField('纳入 IP 地址管理', default=True)
    auto_generate = BooleanField('保存后自动生成该子网内全部可用 IP（不超过 4096 个）', default=False)
    submit = SubmitField('保存')

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        from models.network_models import Vlan
        self.vlan_id.choices = [(0, '无（不关联 VLAN）')] + [
            (v.id, f'{v.vlan_id} - {v.name}') for v in Vlan.query.order_by(Vlan.vlan_id).all()
        ]
        self.location_id.choices = [(0, '无（不关联位置）')] + [
            (l.id, l.name) for l in Location.query.order_by(Location.name).all()
        ]


class IPAddressForm(FlaskForm):
    subnet_id = SelectField('所属子网', coerce=int, validators=[DataRequired(message='请选择子网')])
    ip_address = StringField('IP 地址', validators=[DataRequired(message='请输入 IP 地址'), Length(max=45)])
    status = SelectField('状态', choices=[
        ('available', '空闲'), ('reserved', '预留'),
        ('allocated', '已分配'), ('disabled', '禁用')
    ], default='available')
    device_id = SelectField('关联设备', coerce=int, validators=[Optional()])
    hostname = StringField('主机名', validators=[Optional(), Length(max=128)])
    mac_address = StringField('MAC 地址', validators=[Optional(), Length(max=17)])
    owner = StringField('使用人', validators=[Optional(), Length(max=64)])
    department = StringField('部门', validators=[Optional(), Length(max=100)])
    description = TextAreaField('用途/备注', validators=[Optional()])
    submit = SubmitField('保存')

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        from models.network_models import Subnet
        self.subnet_id.choices = [
            (s.id, f'{s.name} ({s.cidr})') for s in Subnet.query.order_by(Subnet.network).all()
        ]
        self.device_id.choices = [(0, '不关联设备')] + [
            (d.id, f'{d.name} ({d.ip_address or "-"})')
            for d in Device.query.filter_by(is_decommissioned=False).order_by(Device.name).all()
        ]


# ==================== 自动化执行中心表单 ====================

class ExecTaskForm(FlaskForm):
    name = StringField('任务名称', validators=[DataRequired(message='请输入任务名称'), Length(max=128)])
    description = TextAreaField('描述', validators=[Optional()])
    ...
    protocol = SelectField('执行协议', choices=[
        ('auto', '自动判断'),
        ('ssh', 'SSH (Linux/通用)'),
        ('winrm', 'WinRM (Windows)'),
        ('netmiko', 'Netmiko (网络设备)'),
        ('local', '本机执行'),
    ], default='auto')
    content = TextAreaField('命令/脚本内容', validators=[DataRequired(message='请输入命令或脚本内容')],
                            render_kw={'rows': 10, 'placeholder': '每行一条命令；脚本类型支持多行脚本；配置下发按行写入配置模式'})
    timeout = IntegerField('单机超时(秒)', default=30, validators=[
        Optional(), NumberRange(min=5, max=600, message='超时时间 5-600 秒')])
    target_mode = SelectField('目标选择方式', choices=[
        ('devices', '手动选择设备'),
        ('group', '按设备分组'),
        ('subnet', '按子网'),
        ('all', '全部在线设备'),
    ], default='devices')
    device_ids = SelectMultipleField('选择设备', coerce=int, validators=[Optional()])
    group_id = SelectField('设备分组', coerce=int, validators=[Optional()])
    subnet_id = SelectField('子网', coerce=int, validators=[Optional()])
    approval_required = BooleanField('需要审批后执行', default=False)
    auto_run = BooleanField('创建后立即执行', default=True)
    notify_enabled = BooleanField('完成后通知我', default=True)
    schedule_enabled = BooleanField('启用定时执行', default=False)
    schedule_type = SelectField('定时类型', choices=[
        ('interval', '间隔'),
        ('cron', 'Cron 表达式'),
        ('once', '单次（时间点）'),
    ], default='interval')
    schedule_value = StringField('定时值', validators=[Optional(), Length(max=100)],
                                 render_kw={'placeholder': '间隔如 5m/1h/1d；Cron 如 0 3 * * *；单次如 2026-08-16 03:00'})
    submit = SubmitField('保存')

    def validate_device_ids(self, field):
        if self.target_mode.data == 'devices' and not field.data:
            raise ValidationError('请至少选择一台设备')

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        from models.device_group_models import DeviceGroup
        from models.network_models import Subnet
        from models.exec_models import ExecTemplate
        devices = Device.query.filter_by(is_decommissioned=False).order_by(Device.name).all()
        self.device_ids.choices = [
            (d.id, f'{d.name} ({d.management_ip or d.ip_address or "-"})') for d in devices
            if d.management_ip or d.ip_address
        ]
        self.group_id.choices = [(0, '请选择设备分组')] + [
            (g.id, g.name) for g in DeviceGroup.query.order_by(DeviceGroup.name).all()
        ]
        self.subnet_id.choices = [(0, '请选择子网')] + [
            (s.id, f'{s.name} ({s.cidr})') for s in Subnet.query.order_by(Subnet.network).all()
        ]


class ExecTemplateForm(FlaskForm):
    name = StringField('模板名称', validators=[DataRequired(message='请输入模板名称'), Length(max=128)])
    description = TextAreaField('描述', validators=[Optional()])
    task_type = SelectField('任务类型', choices=[
        ('command', '命令执行'),
        ('script', '脚本执行'),
    ...
    timeout = IntegerField('单机超时(秒)', default=30, validators=[
        Optional(), NumberRange(min=5, max=600, message='超时时间 5-600 秒')])
    approval_required = BooleanField('默认需要审批', default=False)
    submit = SubmitField('保存')


class ApiKeyForm(FlaskForm):
    name = StringField('令牌名称', validators=[DataRequired(message='请输入令牌名称'), Length(max=128)])
    scope = SelectField('权限范围', choices=[
        ('read', '只读'),
        ('write', '读写'),
    ], default='read')
